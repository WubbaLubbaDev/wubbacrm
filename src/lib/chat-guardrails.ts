...
// This is the client-side version used by tests and the UI.
// The Edge Function has its own copy in supabase/functions/chat-companion/guardrails.ts.
 ...
 */
export function buildSystemPrompt(chunks: RetrievedChunk[], calendarContext?: string): string {
  const contextText =
    chunks.length > 0
