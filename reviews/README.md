...
### setup/
- `scaffold-auth-review.md` — Phase 0 scaffold + Supabase auth review (full CHANGES REQUESTED → fix → APPROVED history)
- `auth-issues.md` — recurring auth patterns & regression checklist (logout redirect, beforeLoad guards, session retrieval, secrets hygiene)

### style/
- `ui-review.md` — Phase 1 dashboard UI + component library review (with Phase 1.5 login restyle + bug fix history)
- `layout-issues.md` — recurring layout/style patterns & regression checklist (dropdown spacing, search bar, overflow/truncation, hardcoded colors, missing variants, cn() merge, avatar onError)
...
2. Patch relevant issue modules (e.g. add new layout bugs to `layout-issues.md`, new auth bugs to `auth-issues.md`)
...
**Issue modules** (`auth-issues.md`, `layout-issues.md`) are the regression checklists. Each entry has:
...
- `scaffold-auth-review.md` (setup modules)
- `dashboard-ui-review.md` (style modules)
